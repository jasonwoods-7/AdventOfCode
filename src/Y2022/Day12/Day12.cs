...
public class Day12 : IAoCRunner<(Node, Node), int>
{
    public (Node, Node) ParseInput(IEnumerable<string> puzzleInput)
    {
        ...
        static void AddValidAdjacentIfExists(
            Dictionary<(int, int), Node> grid,
            (int, int) key,
            Node current
        )
        {
            if (grid.TryGetValue(key, out var adjacent) && adjacent.Height - current.Height <= 1)
            {
    ...
    )
    {
        ...
        static IEnumerable<Node> PotentialStartPositions(
            Node currentNode,
        ...
        )
        {
            ...
            foreach (
                var adjacent in currentNode.AdjacentNodes.SelectMany(n =>
                    PotentialStartPositions(n, visitedNodes)
                )
            )
            {
