...
{
    public IReadOnlyList<int> ParseInput(IEnumerable<string> puzzleInput) => puzzleInput
        .Select(static line => line.Split('|'))
        .Select(static ss => ss[0]
            .FindNumbers<int>()
            .Skip(1)
            .ToImmutableHashSet()
    ...
    public int RunPart1(IReadOnlyList<int> input) => input
        .Sum(static t => (1 << t) >> 1);

    public int RunPart2(IReadOnlyList<int> input)
    {
        var ticketCount = input
            .Select(_ => 1)
            .ToList();
